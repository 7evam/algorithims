...
// console.log(dfsWithStack(sampleGraph));

/**
 *  2. Implement Depth First Search using recursion. Return all valid paths for a given source and destination.
 *
 *  Input: {Vertex} - the starting vertex
 *  Output: {Array} - an array of arrays of valid paths
 *
...
function validDfs(paths, source, destination) {
  ...
  function testDfs(current, destination) {
    ...
    for (let neighbor of neighbors) {
      if (visited.has(neighbor.id) == false) {
        if (testDfs(neighbor, destination)) { return true; }
       }
    }
    visited.delete(current.id);
    return false;
  ...
  let visited = new Set();
  if (testDfs(source, destination) && paths == []) { return false; }
  return true;
